...
import styles from './../../cardInsert/components/CardInsert.module.scss';
...
export default class CardUpdate extends React.Component<IPropsInsert, IStateInsert> {
  ...
  public constructor(props: IPropsInsert, state: IStateInsert){     
    super(props);
    var itemid = this.logMessageToConsole();
    console.log(`Find ItemID : ${itemid}`);
    ...
    if(itemid){
      this.serviceclass.getWelcomeMessageDetails(this.props.context, `/_api/web/lists/getbytitle('Employee Registration')/Items(${itemid})?$select=*,ReportingManager/ID,ReportingManager/EMail&$expand=ReportingManager`).then((ticketitems: any) => {                
        if(ticketitems){
          let usermanager = [];
          usermanager.push(ticketitems.ReportingManager.EMail);
          this.setState({
            name: ticketitems.Title,
            description: ticketitems.Description,
            usermanageremailid: usermanager,
            termKey: ticketitems.Projects.TermGuid,
          });
        }
      });
    }
  }
  public render(): React.ReactElement<IPropsInsert> {
                      ...
                      <div className={`ms-Grid-row ${styles.row}`}>
                        ...
                        <div className="ms-Grid-col ms-sm6 ms-md3 ms-lg4 block">
                          <PeoplePicker
                            ...
                            defaultSelectedUsers={this.state.usermanageremailid}
                            />
                        </div>
                      </div>
                    </AccordionItemPanel>
  ...
  private logMessageToConsole(): string
  {
    var value = "";
    var queryParms=new UrlQueryParameterCollection(window.location.href);
      if(queryParms.getValue("EID")){
        console.log(queryParms.getValue("EID"));
        value = queryParms.getValue("EID");
    }
    return value;
  }
